// ./components/ImagesCard.js (Example path)
...
const ImageCard = ({ observation, onImageClick }) => { // Added onImageClick prop
  if (!observation) {
    return null;
  }
  ...
  const handleCardImageClick = () => {
    if (hasActualImage && onImageClick) {
      onImageClick(imageUrl, `APT image from ${satelliteName} - Obs ID ${observation.id}`);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md hover:shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-200 ease-in-out hover:-translate-y-1 flex flex-col">
      ...
      {/* Image Section - Modified for consistent size and click */}
      <div
        className="aspect-w-16 aspect-h-9 bg-black/80 dark:bg-black cursor-pointer group" // 16:9 aspect ratio, dark bg for contain
        onClick={handleCardImageClick}
      >
        <img
          src={imageUrl}
          alt={`APT image from ${satelliteName} observation ${observation.id}`}
          // Use object-contain for APT images to see full width, object-cover if you prefer fill/crop
